...
static BTC_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^(1[a-km-zA-HJ-NP-Z1-9]{25,34}|3[a-km-zA-HJ-NP-Z1-9]{25,34}|bc1[a-zA-HJ-NP-Z0-9]{25,59})$").unwrap()
});
static ETH_REGEX: Lazy<Regex> = Lazy::new(|| Regex::new(r"^0x[a-fA-F0-9]{40}$").unwrap());
static LTC_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^[LM3][a-km-zA-HJ-NP-Z1-9]{26,33}$").unwrap());
static XMR_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^4[0-9AB][1-9A-HJ-NP-Za-km-z]{93}$").unwrap());
static DOGE_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^D{1}[5-9A-HJ-NP-U]{1}[1-9A-HJ-NP-Za-km-z]{32}$").unwrap());
